...
if [[ "${PUSH_OPT}" == "true" ]]; then
  ...
  img_name="ghcr.io/snazy/${img_basename}"
...
docker buildx \
  ...
  --annotation "index,manifest:org.opencontainers.image.url=https://github.com/snazy/maxio-daily" \
  --annotation "index,manifest:url=https://github.com/snazy/maxio-daily" \
